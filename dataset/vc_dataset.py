...
if __name__ == '__main__':
    ...
    preprocessed_data_dir='../data_preprocessing/data_preprocessed/time_training'
    boat_A_id,boat_B_id='1','3'
    with open(os.path.join(preprocessed_data_dir, boat_A_id, f"{boat_A_id}_normalized.pickle"), 'rb') as f:
        dataset_A= pickle.load(f)
    with open(os.path.join(preprocessed_data_dir, boat_B_id, f"{boat_B_id}_normalized.pickle"), 'rb') as f:
        dataset_B= pickle.load(f)
    dataset_A_norm_stats = np.load(os.path.join(preprocessed_data_dir, boat_A_id, f"{boat_A_id}_norm_stat.npz"))
    dataset_A_mean = dataset_A_norm_stats['mean']
    dataset_A_std = dataset_A_norm_stats['std']
    dataset_B_norm_stats = np.load(os.path.join(preprocessed_data_dir, boat_B_id, f"{boat_B_id}_norm_stat.npz"))
    dataset_B_mean = dataset_B_norm_stats['mean']
    dataset_B_std = dataset_B_norm_stats['std']
    dataset = VCDataset(datasetA=dataset_A,
                             datasetB=dataset_B,
                             n_frames=128,
                             max_mask_len=25)
    ...
    for i, (A, mask_A, B, mask_B) in enumerate(train_dataloader):
