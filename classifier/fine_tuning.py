...
def smooth(data, sm=1):
    ...
    return np.sum(smooth_data,axis=1)
...
if __name__ =="__main__":
    ...
    if args.expand:
        for i,j in [(1,4),(2,4),(3,4)]:
            ...
            Y_expand=np.zeros([X_expand.shape[0],4])
            Y_expand[:,3]=1e-8
            X_train=np.vstack((X_train,X_expand))
            Y_train=np.vstack((Y_train,Y_expand))
    X_train=np.delete(X_train,0,axis=0)
    Y_train = np.delete(Y_train, 0, axis=0)
    # X_train,X_test=standardization(X_train,X_test)
    traindataset=Mydataset(X_train.reshape([-1,1,128,128]),Y_train)
    testdataset = Mydataset(X_test.reshape([-1,1,128,128]), Y_test)
    train_loader = torch.utils.data.DataLoader(dataset=traindataset,batch_size=args.batch_size,shuffle=True,pin_memory=False,num_workers = 4)
    test_loader = torch.utils.data.DataLoader(dataset=testdataset,batch_size=args.batch_size,shuffle=False,pin_memory=True,num_workers = 4)
    #模型配置
    if args.model_choice == 'resnet34':
        ...
        model = CNN(1, 128)
    model.load_state_dict(torch.load(f'../results/mini_{args.model_choice}.pth'))
            ...
            for i,(audio, labels) in enumerate(test_loader):
                ...
                for i in range(labels.size(0)):
                    if labels[i] == predicted[i]:
                        correct+=1
                        each[labels[i]]+=1
            print('Epoch [{}/{}],  Loss: {:.4f},test acc: {:.2f} %'.format(epoch + 1, args.num_epochs, all_loss,100 * correct / total))
            accs=np.vstack((accs,np.hstack((each/single,correct / total)).reshape(1,-1)))
    # 保存acc曲线，保存模型参数
    plot(accs,f'acc_{"expand" if args.expand else "ori"}')
    torch.save(model.state_dict(), f'../results/mini_{args.model_choice}_{np.average(accs[-10:,-1]):.2f}.pth.tar')
