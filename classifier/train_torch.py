...
def plot(accs, pngname, smoothstep=7):
    '''
    先画各类，再画总acc
    '''
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datadir', type=str, default="data/pickle_time",
    ...
    if args.expand:
        for i, j in [(1, 2), (1, 3), (2, 3)]:
            path = os.path.join(f'../results/mask_cyclegan_lofar_{i}_{j}/converted_audio/converted_{i}_to_{j}.pickle')
            ...
            path2 = os.path.join(f'../results/mask_cyclegan_lofar_{i}_{j}/converted_audio/converted_{j}_to_{i}.pickle')
            X_expand = readdata2(path)
            Y_expand = np.full([X_expand.shape[0]], fill_value=i)
            X_train = np.vstack((X_train, X_expand))
            Y_train = np.hstack((Y_train, Y_expand))
    ...
    torch.save(model.state_dict(),
               f'../results/time_{args.model_choice}_{"expand" if args.expand else "ori"}_{np.average(accs[-10:, -1]):.2f}.pth')
